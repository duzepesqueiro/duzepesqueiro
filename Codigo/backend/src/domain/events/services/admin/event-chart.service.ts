...
export class EventChartService {
  ...
  async getWeeklyChartData(referenceDate?: Date): Promise<IWeeklyChartData> {
    const reference = referenceDate ?? new Date();
    const dateOnly = new Date(reference.getFullYear(), reference.getMonth(), reference.getDate());
    const startOfWeek = new Date(dateOnly);
    startOfWeek.setDate(dateOnly.getDate() - dateOnly.getDay());
    startOfWeek.setHours(0, 0, 0, 0);
    const endOfWeek = new Date(startOfWeek);
    endOfWeek.setDate(startOfWeek.getDate() + 6);
    endOfWeek.setHours(23, 59, 59, 999);

    const cacheKey = `chart:weekly:${startOfWeek.toISOString().slice(0, 10)}`;
    return this.fromCache(cacheKey, async () => {
      ...
      const weekdayParticipants = Array.from({ length: 7 }, () => 0);
      const eventsWithParticipants = await Promise.all(
        events.map(async (event) => ({
          event,
          participants: await this.registrationRepository.countByEventId(event.id),
        })),
      );

      eventsWithParticipants.forEach(({ event, participants }) => {
        const dayIndex = new Date(event.eventDate).getDay();
        weekdayEvents[dayIndex] += 1;
        weekdayParticipants[dayIndex] += participants;
      });

      return {
        days: ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'],
        participants: weekdayParticipants,
        events: weekdayEvents,
        totalParticipants: weekdayParticipants.reduce((acc, value) => acc + value, 0),
        totalEvents: weekdayEvents.reduce((acc, value) => acc + value, 0),
      };
    });
  }
