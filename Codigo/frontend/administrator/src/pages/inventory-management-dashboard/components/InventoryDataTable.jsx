    ...
    try {
      ...
      if (modalType === 'create') {
        if (modalProduct.registrationType === 'Aluguel') {
          const name = String(modalProduct.product || '').trim();
          const hourlyPrice = Number(modalProduct.hourlyPrice);
          const available = Number(modalProduct.available);
          if (!name) {
            throw new Error('Nome do produto é obrigatório.');
          }
          if (!Number.isFinite(hourlyPrice) || hourlyPrice <= 0) {
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <Select
                ...
                onChange={(val) => modalType !== 'view' && setModalProduct(prev => ({
                  ...prev,
                  registrationType: val,
                  category: val === 'Aluguel' ? 'Equipamentos para Aluguel' : prev.category,
                  lastRestocked: val === 'Aluguel' ? '' : prev.lastRestocked,
                }))}
                className="w-full"
              />
